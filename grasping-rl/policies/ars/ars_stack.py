...
class Worker(object):
    ...
    def __init__(self, env_seed,
        ...
        if env_name == 'StickyLiftCubeFree-v0':
            self.env = gym.make('LiftCubeFree-v0')
            import rl2wrapper
            self.env = rl2wrapper.StickyActionEnv(self.env, 10)
        else:
            self.env = gym.make(env_name)
        self.env.seed(env_seed)

        import actionwrapper
        self.env = actionwrapper.stackstatewrapper(self.env, num_states)
        ...
        #elif policy_params['type'] == 'mlp':
        #    import tf_policies
        #    policy_params['ac_space'] = self.env.action_space
        #    policy_params['ob_space'] = self.env.observation_space
    ...
    def do_rollouts(self, w_policy, num_rollouts = 1, shift = 1, evaluate = False):
        ...
        for i in range(num_rollouts):
            ...
            if evaluate:
                ...
                reward, r_steps = self.rollout(shift = 0., rollout_length = self.env.spec.timestep_limit*10)
...
class ARSLearner(object):
    ...
    def aggregate_rollouts(self, num_rollouts = None, evaluate = False):
        ...
        rollout_rewards /= (1e-8 + np.std(rollout_rewards))
    ...
    def train(self, num_iter):
        ...
        for i in range(num_iter):
            ...
            if ((i + 1) % 10 == 0):
                ...
                reward_dict.append(np.mean(rewards))
                timesteps_dict.append(self.timesteps)
                np.save(self.logdir + '/timesteps', timesteps_dict)
                np.save(self.logdir + '/reward', reward_dict)
                np.save(self.logdir +'/params', w)
...
def run_ars(params):
    ...
    logdir = 'data/env_{}/seed_{}std_{}stepsize_{}policy_{}numstates_{}'.format(params['env_name'],params['seed'],params['delta_std'],
               params['step_size'],params['policy_type'],params['numstates'])#dir_path
    if not(os.path.exists(logdir)):
        os.makedirs(logdir)

    if params['env_name'] == 'StickyLiftCubeFree-v0':
        env = gym.make('LiftCubeFree-v0')
        env = rl2wrapper.StickyActionEnv(env, 10)
    else:
        env = gym.make(params['env_name'])
    env = actionwrapper.stackstatewrapper(env, params['numstates'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--numstates', type=int, default=1)

    local_ip = socket.gethostbyname(socket.gethostname())
    #ray.init(redis_address= local_ip + ':8101')
    ray.init()
    ...
    print("FINISHED: ",time.time() - time0)
